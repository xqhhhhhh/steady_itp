...
def vpn_switch(payload: VpnSwitchRequest) -> VpnSwitchResponse:
    ...
    if not commands:
        return VpnSwitchResponse(
            ok=False,
            message="VPN_SWITCH_COMMANDS 未配置，无法自动切换",
        )
    ...
    if not success:
        return VpnSwitchResponse(
            ok=False,
            switched_index=_vpn_last_index,
    ...
    return VpnSwitchResponse(
        ...
        switched_command=cmd,
        verify_ok=verify_ok,
        message=f"已执行切换; verify={verify_reason}; reason={payload.reason}; queue={payload.current_queue}",
    )
